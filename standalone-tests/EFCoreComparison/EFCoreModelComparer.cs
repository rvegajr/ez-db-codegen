...
namespace StandaloneEFCoreComparison
{
    ...
    public class EFCoreModelComparer
    {
        ...
        /// <summary>
        /// Compares EF Core and EzDbCodeGen performance
        /// </summary>
        /// <param name="dbContext">The EF Core DbContext</param>
        /// <returns>Performance metrics for both approaches</returns>
        public PerformanceMetrics ComparePerformance(DbContext dbContext)
        {
            _logger.LogInformation("Comparing EF Core and EzDbCodeGen performance...");
            
            var metrics = new PerformanceMetrics();
            
            // Measure EF Core performance
            var stopwatch = System.Diagnostics.Stopwatch.StartNew();
            var relationships = AnalyzeEFCoreRelationships(dbContext);
            stopwatch.Stop();
            metrics.EFCoreRelationshipAnalysisTime = stopwatch.ElapsedMilliseconds;
            
            stopwatch.Restart();
            var columnTypes = AnalyzeEFCoreColumnTypes(dbContext);
            stopwatch.Stop();
            metrics.EFCoreColumnTypeAnalysisTime = stopwatch.ElapsedMilliseconds;
            
            metrics.EFCoreRelationshipCount = relationships.Count;
            metrics.EFCoreColumnCount = columnTypes.Count;
    ...
    {
        public long EFCoreRelationshipAnalysisTime { get; set; }
        public long EFCoreColumnTypeAnalysisTime { get; set; }
        public int EFCoreRelationshipCount { get; set; }
        public int EFCoreColumnCount { get; set; }
        
        public override string ToString()
        {
            return $"EF Core: {EFCoreRelationshipCount} relationships in {EFCoreRelationshipAnalysisTime}ms, " +
                   $"{EFCoreColumnCount} columns in {EFCoreColumnTypeAnalysisTime}ms";
        }
    }
}
